...
module.exports = class HybridCryptography {
	...
	*/
	...
	hybridDecrypt(algorithm, private_key, passphrase, aes_key, aes_iv, rsa_next_pub, enc_body) {
		try{
			...
			if (enc_body) {
                ...
                body = this.aesDecrypt(algorithm, key, iv, enc_body_bytes)
			}
	...
	hybridEncrypt(algorithm, rsa_modulus_length, public_key, body) {
		try{
			let is_json = false;
			...
			//now, lets create the next RSA public key to use (Double Ratchet)
            const date = new Date();
            //lets create our own new key sets for this session. The passphrase is the created date
            const rsa_keys = this.createRSAEncrytptionKey(date.getTime(), rsa_modulus_length, algorithm);
			//encrypt the new rsa public key
