...
app.post("/auth/handshake", (req: any, res: any) => {
	try {
		...
		const result = serverHS.handleHandshakeRequest(req.headers, session_id);
		...
		console.error(err);
		res.send((err as Error).message, 500);
	}
});
...
app.post("/test", (req: any , res: any) => {
	try {
		...
		const decReq = serverHS.decryptRequest(req.headers, req.body, privateKey, passphrase);
		...
		response = serverHS.encryptResponse(req.headers.swhs_sess_id, resBody, decReq.next_pub);
		// store the next request decryption items in the session
